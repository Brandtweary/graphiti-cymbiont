    ...
    match provider:
        case GraphProvider.NEPTUNE:
            return """
                ...
                entity_edges: join([x IN coalesce($entity_edges, []) | toString(x) ], '|'), created_at: $created_at, valid_at: $valid_at, metadata: $metadata}
        ...
        case GraphProvider.FALKORDB:
            return """
                MERGE (n:Episodic {uuid: $uuid})
                SET n = {uuid: $uuid, name: $name, group_id: $group_id, source_description: $source_description, source: $source, content: $content,
                entity_edges: $entity_edges, created_at: $created_at, valid_at: $valid_at, metadata: $metadata}
                RETURN n.uuid AS uuid
            """
        case _:  # Neo4j
            return """
    ...
    match provider:
        case GraphProvider.NEPTUNE:
            return """
                ...
                entity_edges: join([x IN coalesce(episode.entity_edges, []) | toString(x) ], '|'), created_at: episode.created_at, valid_at: episode.valid_at, metadata: episode.metadata}
                RETURN n.uuid AS uuid
            """
        case GraphProvider.KUZU:
            return """
        ...
        case GraphProvider.FALKORDB:
            return """
                ...
                entity_edges: episode.entity_edges, created_at: episode.created_at, valid_at: episode.valid_at, metadata: episode.metadata}
                RETURN n.uuid AS uuid
            """
        case _:  # Neo4j
            return """
